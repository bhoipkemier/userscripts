...
(function() {
	...
	const css = `
		...
		.tblAvailability th { background-color: #f9f9f9; }
		...
		.peopleReq { background-color: rgb(214,233,198); border: 1px solid rgb(60,118,61); }
		.unAvail { background-color: rgb(235,204,209); border: 1px solid rgb(169,68,66); }
	`;
	...
	var workersLoaded = function() {
		...
		curWorkers.forEach(w => {
			const a = $('<a href="#" class="list-group-item person"/>').text(flipName(w.name));
	...
	var getEventRow = function(event){
		...
		getEventPositions(event).forEach(position => {
			...
			position.assignments.forEach(assignment => {
				var passign = $('<a href="" class="passign person list-group-item"/>')
					.text(assignment.volunteer.individual.name)
					.append('<a href="javascrip:void" class="btn badge"><i class="glyphicon glyphicon-trash removePassign"></i></a>')
